...
describe('Roo Initialization Functionality', () => {
    ...
    test('init.js creates Roo directories in createProjectStructure function', () => {
        ...
        const hasRooDir = initJsContent.includes("ensureDirectoryExists(path.join(targetDir, '.roo'))");
        expect(hasRooDir).toBe(true);
        
        // Check for the line that creates .roo/rules directory
        const hasRooRulesDir = initJsContent.includes("ensureDirectoryExists(path.join(targetDir, '.roo', 'rules'))");
        expect(hasRooRulesDir).toBe(true);
        
        // Check for the for loop that creates mode-specific directories
        const hasRooModeLoop = initJsContent.includes("for (const mode of ['architect', 'ask', 'boomerang', 'code', 'debug', 'test'])") ||
    ...
    test('init.js copies Roo files from assets/roocode directory', () => {
        ...
        const copiesRoomodes = initJsContent.includes("copyTemplateFile('.roomodes'");
    ...
    test('init.js has code to copy rule files for each mode', () => {
        ...
        const hasModeRulesCopying = initJsContent.includes("copyTemplateFile(") &&
