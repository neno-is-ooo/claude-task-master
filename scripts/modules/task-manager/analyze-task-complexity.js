 ...
 */
function generateStandardPrompt(tasksData) {
 ...
 * @returns {string} The generated prompt.
 */
function generateBalancedPrompt(tasksData) {
	const tasksString = JSON.stringify(tasksData.tasks, null, 2);
 ...
 * @returns {string} The generated prompt.
 */
function generateAdvancedPrompt(tasksData) {
	const tasksString = JSON.stringify(tasksData.tasks, null, 2);
...
/**
 * Generates the prompt for complexity analysis based on the configured mode.
 * @param {Object} tasksData - The tasks data object.
 * @param {string} mode - The complexity mode ('standard', 'balanced', 'advanced').
 * @returns {string} The generated prompt.
 */
function generateInternalComplexityAnalysisPrompt(tasksData, mode = null) {
	// If no mode provided, get from config
	if (!mode) {
		mode = getComplexityMode();
	}
	...
	switch (mode) {
		...
		default:
			log('warn', `Unknown complexity mode: ${mode}, using balanced`);
			return generateBalancedPrompt(tasksData);
	}
}
