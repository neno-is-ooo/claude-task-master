...
const conversionConfig = {
  ...
  brandTerms: [
    ...
    { from: /\bcursor\b/gi, to: (match) => match === 'Cursor' ? 'Roo Code' : 'roo' },
  ...
  docUrls: [
    { from: /https:\/\/docs\.cursor\.com\/[^\s)'"]+/g, to: (match) => match.replace('docs.cursor.com', 'docs.roocode.com') },
...
function transformCursorToRooRules(content) {
  ...
  result = result.replace(/https?:\/\/www\.cursor\.so/gi, 'https://www.roocode.com');
  ...
  result = result.replace(/\bcursor\b/gi, (match) => match.charAt(0) === 'C' ? 'Roo Code' : 'roo');
...
function convertCursorRuleToRooRule(sourcePath, targetPath) {
  try {
    log('info', `Converting Cursor rule ${path.basename(sourcePath)} to Roo rule ${path.basename(targetPath)}`);
    ...
    log('success', `Successfully converted ${path.basename(sourcePath)} to ${path.basename(targetPath)}`);
    
    return true;
  } catch (error) {
    log('error', `Failed to convert rule file ${path.basename(sourcePath)}: ${error.message}`);
    return false;
  }
}
...
function convertAllCursorRulesToRooRules(projectDir) {
  ...
  log('info', `Rule conversion complete: ${success} successful, ${failed} failed`);
